...
async function retrieveWorkoutImages(id) {
  ...
  if (!response.ok) {
    ...
    document.getElementById("workout-title").innerHTML =
      "Workout name: " + workoutData["name"]; //8
    document.getElementById("workout-owner").innerHTML =
      "Owner: " + workoutData["owner_username"]; //9
    ...
    for (let fileData of workoutData.files) {
      //18
      let a = document.createElement("a"); //19
      a.href = fileData.file; // 20
      let pathArray = fileData.file.split("/"); //21
...
async function validateImgFileType(id, host_variable, acceptedFileTypes) {
  let file = await sendRequest(
    "GET",
    `${host_variable}/api/workout-files/${id}/`
  ); //Rule 8: endpoint should not be hard-coded
  let fileData = await file.json();
  let fileType = fileData.file
    .split("/")
...
window.addEventListener("DOMContentLoaded", async () => {
  let goBackButton = document.querySelector("#btn-back-workout");
